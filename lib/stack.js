...
function Stack(options) {
	...
	self.digitalOcean = options.digitalOcean ? options.digitalOcean : {};
	self.namecheap = options.namecheap ? options.namecheap : {};
	...
	self.on("load", function() {
		console.log("[STACK] " + self.server.name + " loaded from disk".gray);
	});
	self.on("refresh", function() {
		//console.log("[STACK] Droplet refreshed");
	});
	self.on("create", function() {
		console.log("[STACK] Created: " + self.server.name.green);
	});
	self.on("ready", function() {
		console.log("[STACK] Ready!".gray);
	});
	self.on("save", function() {
		console.log("[STACK] Saved".gray);
	});
	self.on("keygen", function() {
		console.log("[STACK] Keys generated".green);
	});
	self.on("unkeygen", function() {
		console.log("[STACK] Keys deleted".green);
	});
	self.on("destroy", function() {
		console.log("[STACK] Destroyed: " + self.server.name.green);
	});
	self.on("error", function(err) {
		console.log("[STACK] Error!".red);
		console.log(err);
	});
...
Stack.prototype.sendFiles = function(dests) {
	...
	return new Promise(function(resolve, reject) {
					...
					funcs.push(function(callback) {
						...
						output.on("finish", function(err) {
							if (err) {
								...
								console.log("[STACK] SFTP upload: " + dest.green);
								callback(null);
							}
						});
						output.end(compiled);
		...
		conn.on("end", function() {
			console.log("[STACK] SFTP disconnect".green);
			resolve();
		});
		var trys = 0;
		conn.on("error", function(err) {
			if (trys < 10 && err.level === "client-socket") {
				trys++;
				console.log("[STACK] SFTP timeout " + trys + "/10, retrying...".yellow);
				clear = setTimeout(function() {
					conn.connect(host);
				}, 10000);
			} else if (trys >= 10) {
				console.log("[STACK] SFTP timeout for good".red);
				reject(err);
			} else {
				self.emit("error", err);
				reject(err);
	...
	return new Promise(function(resolve, reject) {
		...
		function doIt() {
			...
			SSH.on("connect", function onConnect() {
				console.log("[STACK] shh connected to " + self.server.name.green);
				if (clear) {
					global.clearTimeout(clear);
				}
			});
			SSH.on("ready", function onReady() {
				console.log("[STACK] running " + commands.length + " commands".gray);
			});
			SSH.on("commandTimeout", function onCommandTimeout(command, response, stream, connection) {
				console.log("[STACK] SSH timeout on: " + command.red);
				reject(new Error("timed out on: " + command));
			});
			SSH.on("end", function onEnd() {
				console.log("[STACK] SSH commands end".green);
				resolve();
			});
			SSH.on("close", function onClose(had_error) {
				var errString = had_error ? "d with error: " + had_error : "";
				console.log("[STACK] SSH close" + errString.gray);
			});
			SSH.on("error", function onError(err, type, close, callback) {
				console.log("[STACK] SSH error: " + type.red);
				if (clear) {
					global.clearTimeout(clear);
				}
				if (close && trys >= 10) {
					console.log("[STACK] SSH connect timed out with 10 retrys".red);
					reject(err);
				} else if (close && trys < 10) {
					trys++;
					console.log("[STACK] SSH connect timeout " + trys + "/10, retrying...".yellow);
...
Stack.prototype.setDNS = function() {
	var self = this;
	return new Promise(function(resolve, reject) {
		...
		request.get({
				...
				qs: {
					...
					HostName1: "@",
					RecordType1: "A",
					Address1: self.server.droplet.networks.v4[0].ip_address,
					TTL1: "100"
				}
			},
			function(err, res, data) {
				xml2js(data, function(err, result) {
					...
					if (_.has(result, "ApiResponse.CommandResponse[0].DomainDNSSetHostsResult[0].Warnings")) {
						_.each(result.ApiResponse.CommandResponse[0].DomainDNSSetHostsResult[0].Warnings, function(warning) {
							if (warning.Warning[0]._ === "This domain is not currently setup to use Hosts Records.") {
								console.log("[STACK] You can't set DNS on domains that are not controlled by Namecheap's NS".yellow);
							}
						});
					}
					resolve();
